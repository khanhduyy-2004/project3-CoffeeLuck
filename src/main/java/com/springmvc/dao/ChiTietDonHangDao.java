...
public class ChiTietDonHangDao {
    ...
    public int save(ChiTietDonHang ctdh) {
        String sql = "INSERT INTO LTKD_ChiTietDonHang (ltkd_don_hang_id, ltkd_san_pham_id, ltkd_so_luong, ltkd_gia) "
                   + "VALUES (?, ?, ?, ?)";
    ...
    public int update(ChiTietDonHang ctdh) {
        String sql = "UPDATE LTKD_ChiTietDonHang SET ltkd_don_hang_id=?, ltkd_san_pham_id=?, ltkd_so_luong=?, ltkd_gia=? "
                   + "WHERE ltkd_id=?";
        return template.update(sql, ctdh.getDonHangId(), ctdh.getSanPhamId(), ctdh.getSoLuong(), ctdh.getGia(), ctdh.getId());
    }
